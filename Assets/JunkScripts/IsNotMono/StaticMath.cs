...
public class StaticMath
{
    ...
    public Vector3 PointToCellCenterRounded(Vector3 somePoint, Vector3 extents)
    {
        return new Vector3(Mathf.Ceil(somePoint.x) - extents.x, HeightToHalfRoundFloat(RoundByThousand(somePoint.y)) - extents.y, Mathf.Ceil(somePoint.z) - extents.z);
    }

    public float RoundByThousand(float floatCut)
    {
        return Convert.ToSingle(Mathf.Round(floatCut * 100)) / 100f;
    }
...
//        Debug.Log(height.y + " " + height.y % 1f + " " + (height.y % 1f > .5f));
        height.y = height.y.Equals(0.0f) ? 0 : (Mathf.Abs(height.y % 1f) <= .5f && Mathf.Abs(height.y % 1f) > 0 ? Mathf.Ceil(height.y) - .5f : Mathf.Ceil(height.y));
//        Debug.Log(height.y);
        return height;
    }
